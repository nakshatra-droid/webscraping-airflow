...
from core.constants.constants_values import FlipkartConstants
from core.utils.flipkart_queries import (
    FETCH_ACTIVITY_URLS,
...
class FlipkartDBHelpers:
    ...
    def create_scraping_run() -> uuid.UUID:
        """
        Insert a new scraping_metadata row and return its UUID.
        UUID generated in Python — the model has no server-side default for id.
        """
        run_id = uuid.uuid4()
    ...
    def finalise_scraping_run(
        ...
        """Write final counts and end_time to the scraping_metadata row."""
        now = FlipkartDBHelpers.utcnow_naive()
        hook = FlipkartDBHelpers.get_hook()
        hook.run(
            UPDATE_SCRAPING_RUN,
    ...
    def upsert_product(cur, normalised: dict) -> uuid.UUID:
        """
        INSERT into products; on model_number conflict → UPDATE mutable fields.
        Returns product UUID (new or existing).
    ...
    def upsert_product_details(
        ...
        ON CONFLICT on (product_id, source) → UPDATE listing data.
        Constraint name: product_details_product_id_source_key  (from the model).
        """
        now = FlipkartDBHelpers.utcnow_naive()
    ...
    def log_activity(
        ...
        cur.execute(
            ...
            (
                str(run_id),
                activity_type,
                json.dumps(product_data, default=FlipkartDBHelpers._json_default)
    ...
    def flush_activity_buffer(cur, activity_buffer: list[dict]) -> bool:
        ...
        rows = [
            (
                ...
                item["activity_type"],
                json.dumps(
                    item.get("product_data"), default=FlipkartDBHelpers._json_default
                )
    ...
    def flush_product_buffer(
        ...
        try:
            for normalised in product_buffer:
                product_id = FlipkartDBHelpers.upsert_product(cur, normalised)
                now = FlipkartDBHelpers.utcnow_naive()
                cur.execute(
                    query,
                    (
